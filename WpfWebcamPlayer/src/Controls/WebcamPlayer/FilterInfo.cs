...
namespace CatenaLogic.Windows.Presentation.WebcamPlayer
{
	...
	public class FilterInfo : IComparable
	{
		...
		public string MonikerString { get { return _monikerString; } }
		...
		internal static IBaseFilter CreateFilter( string filterMoniker )
		{
			...
			if( 0 == CreateBindCtx( 0, out bindCtx ) )
			{
				// Parse the display name
				if( 0 == MkParseDisplayName( bindCtx, filterMoniker, ref n, out moniker ) )
				{
					// Bind to the object
		...
		private static string GetName(string monikerString)
		{
			...
			{
				// Parse the display name
				if( 0 == MkParseDisplayName( bindCtx, monikerString, ref n, out moniker ) )
				{
					// Get the name
