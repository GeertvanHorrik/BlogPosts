...
namespace CatenaLogic.Windows.Presentation.WebcamPlayer
{
	...
	{
		/// <summary>Maximum video height; </summary>
		const int configMaxHeight = 1080;
		const double configMinFPS = 10;
		...
		private void RunWorker()
		{
			try
			{
				...
				using( AMMediaType mediaType = new AMMediaType() )
				{
					...
					IPin pinInput = _grabberObject.GetPin( PinDirection.Input, 0 );
					IPin pinOutput = _sourceObject.GetPin( PinDirection.Output, 0 );

					ChooseBestFormat( pinOutput );

					if( _graph.Connect( pinOutput, pinInput ) >= 0 )
					{
		...
		private static bool ChooseBestFormat( IPin src )
		{
			/* int configMaxHeight = 1080;
			double configMinFPS = 10;

			string str = ConfigurationManager.AppSettings[ "CapDevice.maxVideoHeight" ];
			if( !string.IsNullOrEmpty( str ) )
				configMaxHeight = int.Parse( str );

			str = ConfigurationManager.AppSettings[ "CapDevice.minFrameRate" ];
			if( !string.IsNullOrEmpty( str ) )
				configMinFPS = double.Parse( str ); */

			IAMStreamConfig cfg = src as IAMStreamConfig;
			if( null == cfg )
				return false;

			int piCount, piSize;
			if( cfg.GetNumberOfCapabilities( out piCount, out piSize ) < 0 )
				return false;

			int maxHeight = 0;
			int maxHeightIndex = -1;

			IntPtr pointer = Marshal.AllocHGlobal(piSize);
			VideoStreamConfigCaps vscf = new VideoStreamConfigCaps();
			try
			{
				AMMediaType mt = null;
				for( int i = 0; i < piCount; i++ )
				{
					cfg.GetStreamCaps( i, out mt, pointer );
					Marshal.PtrToStructure( pointer, vscf );

					double FPS = 1.0 / ( (double)vscf.MinFrameInterval * 100E-9 );
					if( FPS > configMinFPS && vscf.InputSize.cy <= configMaxHeight )
					{
						if( vscf.InputSize.cy > maxHeight )
						{
							maxHeight = vscf.InputSize.cy;
							maxHeightIndex = i;
						}
					}
					// Debug.WriteLine( "Size {0}, FPS {1}", vscf.InputSize.ToString(), FPS );

					mt.Dispose();
				}

				if( maxHeightIndex < 0 )
					return false;	// Didn't find the desired format

				mt = null;
				cfg.GetStreamCaps( maxHeightIndex, out mt, pointer );
				Marshal.PtrToStructure( pointer, vscf );

				Debug.Print( "Capture format: {0}", vscf.InputSize.ToString() );
				cfg.SetFormat( mt );
				mt.Dispose();
			}
			finally
